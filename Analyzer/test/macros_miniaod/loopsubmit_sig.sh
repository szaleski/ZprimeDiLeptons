...
while [ $n -lt ${nlines} ]; do
  ...
  if [ $4 = ${SCERN} ]; then
     ...
     cat submit_ZprimeMuMuAnalysis_DESY.sh | sed "s?which?sig?g" | sed "s?site?$1?g" | sed "s?mc?$3?g" |sed "s?year?$2?g" | sed "s?jobdir?jobs/jobsZprimeMuMu?g" | sed "s?histodir?histos/histosZprimeMuMu?g" | sed "s?output?output_${samplename}?g" | sed "s?sig_input.txt?SigCards$3/sig_input_${n}.txt?g" | sed "s?s.log?s_${samplename}.log?g" > jobs/submit_ZprimeMuMuAnalysis_${samplename}.sh
  elif  [ $4 = ${SBARI} ]; then
