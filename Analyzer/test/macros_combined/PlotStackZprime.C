...
//   Color_t h400Color = kRed;
  ...
  if (histlabel.find("ZprimeRecomass")<20 && nRebin==20){
    hframe= new TH2F("hframe","hframe",80,60.,3000.,500,0.005,50000.);// di-mu  mass nrebin=10  GeV
    hframe2= new TH2F("hframe2","hframe2",80,60.,3000.,1000, -1.5, 3.5);// di-mu mass
  }

  if (histlabel.find("ZprimeRecomass")<20 && nRebin==5){
    hframe= new TH2F("hframe","hframe",80,60.,3000.,500,0.0005,1500.);// di-mu  mass nrebin=5  GeV
    hframe2= new TH2F("hframe2","hframe2",80,60.,3000.,1000, 0.5, 2.);// di-mu mass
  }   
  ...
  for ( int datasetId=Vdatasetnamebkg.size()-1; datasetId >=0; datasetId--){  
    ...
    if(datasetnamebkg.find("WZ")   < 200 ||
       ...
       datasetnamebkg.find("DYJetsToTauTau") < 200 ||
	...
	if(datasetnamebkg.find("WW") < 200){
	  ...
	  if(datasetnamebkg.find(temp) < 200 && (datasetnamebkg.find(whichenergy) < 200 || datasetnamebkg.find(whichsample) < 200) && hfourlepbestmass_4l_afterSel_new_diBoson->GetEntries()>0. ) legend->AddEntry(hfourlepbestmass_4l_afterSel_new_diBoson,"di-boson", "F");
	...
	if(datasetnamebkg.find("_WJets" && useWJetsFromFakeRateFromMC==false ) < 200){
	  cout << "Wjets" << endl;
...
void PlotStackZprime::setSamplesNames4l()
{
  ...
  if (inputfile.find("4l")<25) {
    ...
    whichchannel="#mu#mu";
    histosdir="histos/histosZprimeMuMu_combined";
  };
  ...
  while(std::getline(infile,inputfilename)){
	...
	Vcolorbkg.push_back(kAzure+2);
      ...
      // DYJetsToTauTau_M-50_TuneCUETP8M1_13TeV
      if( inputfilename.find("DYJetsToTauTau_M-50_TuneCUETP8M1_13TeV")<200){
        Vdatasetnamebkg.push_back(inputfilename);
        Vlabelbkg.push_back("DY #rightarrow #tau #tau, m_{#tau #tau}>50 GeV");
        Vxsectionbkg.push_back(1.); //pb
        Vcolorbkg.push_back(kAzure+4);
      }
