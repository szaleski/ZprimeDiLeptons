...
void ZprimeMuMuPat::Loop()
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	//cout << "PT1= " << PtRecTunePMuBestTrack1 << " PT2= " << PtRecTunePMuBestTrack2 << " MassInv= " << MassInv << endl;

	//PrintEventInformation(256843,465,665539990,vtxChi2Mu,vtxMassMu,CosmicRejec);
	h1_ptHistoPassingHLT_->Fill(PtRecTunePMuBestTrack1,weight);
	h1_etaHistoPassingHLT_->Fill(EtaRecMu1,weight);
    ...
    if(vtxChi2Mu<=20.0) 
      {
	h1_ptHistoPassingVtxChi2Mu_->Fill(PtRecTunePMuBestTrack1,weight);
	h1_etaHistoPassingVtxChi2Mu_->Fill(EtaRecMu1,weight);
	float alpha = PI - acos(CosmicRejec);
	//if(alpha>0.001 && alpha<=1) h1_3DangleHisto1_->Fill(alpha,weight);
	//if(alpha>1)  h1_3DangleHisto2_->Fill(alpha,weight);
	h1_3DangleHisto1_->Fill(log10(alpha),weight);
	h1_3DangleHisto2_->Fill(log10(alpha),weight);
	//
	if(CosmicRejec<-0.9998)
	  {
	    h1_Fail3DangleHistoMass_->Fill(vtxMassMu,weight);
	    h1_Fail3DangleHistoPhi_->Fill(PhiRecMu1,weight);
	    
	  }
      }
    if(CosmicRejec>=-0.9998) 
      {
	h1_ptHistoPassingCosmicRejec_->Fill(PtRecTunePMuBestTrack1,weight);
	h1_etaHistoPassingCosmicRejec_->Fill(EtaRecMu1,weight);
      }
  }
  //===================================================
  //                                                  =
...
//                                                   -  
//----------------------------------------------------
float ZprimeMuMuPat::delR(float eta1,float phi1,float eta2,float phi2){
  float mpi=3.14;
...
bool ZprimeMuMuPat::GenRecoMatchMu(float RecoEta1,float RecoPhi1){
  int NbHEEPele = 0;
  unsigned iflag = -10;
...
bool ZprimeMuMuPat::SelectFirstMuon(float &pTmuon1,float &Enmuon1,float &Etamuon1,
  ...
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    ...
    cout << Mu_isTrackerMuon->at(i) << " " <<
      ...
      Mu_ptcocktail->at(i) << " " <<
      Mu_absdxy->at(i) << " " <<
      (Mu_trackiso->at(i)/Mu_ptInnerTrack->at(i)) << " " <<
      Mu_numberOftrackerLayersWithMeasurement->at(i) << " " << 
      Mu_numberOfValidPixelHits->at(i) << " " << 
  ...
  if( NbHEEPele > 0 ){
    FlagMu1             = iflag;
    pTmuon1             = Mu_ptcocktail->at(iflag);
    Enmuon1             = Mu_en->at(iflag);
    Etamuon1            = Mu_etaCocktail->at(iflag);
    Phimuon1            = Mu_phiCocktail->at(iflag);
    //ChargeMu1           = Mu_chargeCocktail->at(iflag);
    ChargeMu1           = Mu_chargeTunePMuonBestTrack->at(iflag);
    pxmuon1             = Mu_pxCocktail->at(iflag);
    pymuon1             = Mu_pyCocktail->at(iflag);
    pzmuon1             = Mu_pzCocktail->at(iflag);
    pmuon1              = Mu_pCocktail->at(iflag);
...
bool ZprimeMuMuPat::SelectSecondMuon(int ChargeMu1,unsigned FlagMu1,float pTmuon1,float Etamuon1,float Phimuon1,
				     ...
				     float &pTmuon2tuneP,float &pTmuonBestTrack2)
{
  ...
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    ...
    if(Mu_ptcocktail->at(i) == pTmuon1) continue;
    if(Mu_etaCocktail->at(i) == Etamuon1) continue;
    if(Mu_phiCocktail->at(i) == Phimuon1) continue;
    //cout << "Charge=" << ChargeMu1 << " " << Mu_chargeTunePMuonBestTrack->at(i) << " pT= " <<  Mu_ptMuonBestTrack->at(i) <<  endl;
    if(ChargeMu1*Mu_chargeCocktail->at(i)>0) continue;
    //if(ChargeMu1*Mu_chargeCocktail->at(i)<0) continue;
    if( Mu_isTrackerMuon->at(i) == 1 &&
	Mu_isGlobalMuon->at(i) == 1 &&
	Mu_ptcocktail->at(i) > 53.0 &&
  ...
  if( NbHEEPele > 0 ){
    pTmuon2          = Mu_ptcocktail->at(iflag);
    Enmuon2          = Mu_en->at(iflag);
    Etamuon2         = Mu_etaCocktail->at(iflag);
    Phimuon2         = Mu_phiCocktail->at(iflag);
    ChargeMu2        = Mu_chargeCocktail->at(iflag);
    pxmuon2          = Mu_pxCocktail->at(iflag);
    pymuon2          = Mu_pyCocktail->at(iflag);
    pzmuon2          = Mu_pzCocktail->at(iflag);
    pmuon2           = Mu_pCocktail->at(iflag);
    dxymuon2         = Mu_absdxy->at(iflag);
...
void ZprimeMuMuPat::PickThehighestMass(float &vtxHighestMass,float &vtxHighestChi2,int EvtNb)
{
  float Massinv  = -10.0;
  unsigned iflag = -10;
  vtxHighestMass = -10.0;
  ...
  for(unsigned i=0; i<Mu_vtxMass->size(); i++)
	...
	if (Mu_ptcocktail->at(j)==Mu_vtxMassLept->at(countlept+1)) chargepair=chargepair*Mu_chargeCocktail->at(j);
      }
...
void ZprimeMuMuPat::MuonPassingID(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaCocktail->at(i)) < 2.4 &&
       ...
       Mu_numberOfMatchedStations->at(i) > 1 &&
       Mu_dPToverPTcocktail->at(i) < 0.3 ){
      h1_PtID_->Fill(Mu_ptcocktail->at(i),weight);
      h1_EtaID_->Fill(fabs(Mu_etaCocktail->at(i)),weight);
...
void ZprimeMuMuPat::PlotPterror(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaCocktail->at(i)) < 2.4 &&
       ...
       Mu_numberOfValidMuonHits->at(i) > 0 &&
       Mu_numberOfMatchedStations->at(i) > 1){
      h1_dPToverPT_->Fill(Mu_dPToverPTcocktail->at(i),weight );
      h1_PtEffpterror_->Fill(Mu_ptcocktail->at(i),weight);
      h1_EtaEffpterror_->Fill(fabs(Mu_etaCocktail->at(i)),weight);
    }
    else continue;
  }
}
...
void ZprimeMuMuPat::PlotNbTrackLayers(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaCocktail->at(i)) < 2.4 &&
      ...
      h1_PtEffptnumberOftrackerLayers_->Fill(Mu_ptcocktail->at(i),weight);
      h1_EtaEffptnumberOftrackerLayers_->Fill(fabs(Mu_etaCocktail->at(i)),weight);
    }
    else continue;
  }
}
...
void ZprimeMuMuPat::PlotNBValidPixelHits(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaCocktail->at(i)) < 2.4 &&
      ...
      h1_PtEffptnumberOfPixelHits_->Fill(Mu_ptcocktail->at(i),weight);
      h1_EtaEffptnumberOfPixelHits_->Fill(fabs(Mu_etaCocktail->at(i)),weight);
    }
    else continue;
  }
}
...
void ZprimeMuMuPat::PlotNbValidMuonHits(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaCocktail->at(i)) < 2.4 &&
      ...
      h1_PtEffptnumberOfMuonHits_->Fill(Mu_ptcocktail->at(i),weight);
      h1_EtaEffptnumberOfMuonHits_->Fill(fabs(Mu_etaCocktail->at(i)),weight);
    }
    else continue;
  }
}
...
void ZprimeMuMuPat::PlotNbMatchedStations(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaCocktail->at(i)) < 2.4 &&
      ...
      h1_PtEffptnumberOfMatchedStations_->Fill(Mu_ptcocktail->at(i),weight);
      h1_EtaEffptnumberOfMatchedStations_->Fill(fabs(Mu_etaCocktail->at(i)),weight);
    }
    else continue;
  }
}
...
void ZprimeMuMuPat::PlotTrackiso(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaCocktail->at(i)) < 2.4 &&
       ...
       Mu_numberOfMatchedStations->at(i) > 1 &&
       Mu_dPToverPTcocktail->at(i) < 0.3 ){
      h1_trackiso_->Fill( Mu_trackiso->at(i)/Mu_ptcocktail->at(i),weight );
      h1_PtEffptTrackIso_->Fill(Mu_ptcocktail->at(i),weight);
      h1_EtaEffptTrackIso_->Fill(fabs(Mu_etaCocktail->at(i)),weight);
    }
    else continue;
  }
}
...
void ZprimeMuMuPat::PlotAbsDxy(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaCocktail->at(i)) < 2.4 &&
       ...
       Mu_numberOfMatchedStations->at(i) > 1 &&
       Mu_dPToverPTcocktail->at(i) < 0.3 ){
      h1_absdxy_->Fill( Mu_absdxy->at(i) ,weight);
      h1_PtEffptabsdsy_->Fill(Mu_ptcocktail->at(i),weight);
      h1_EtaEffptabsdsy_->Fill(fabs(Mu_etaCocktail->at(i)),weight);
    }
    else continue;
  }
}
...
void ZprimeMuMuPat::MuonPassingNewID(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaCocktail->at(i)) < 2.4 &&
       ...
       Mu_numberOfMatchedStations->at(i) > 1 &&
       Mu_dPToverPTcocktail->at(i) < 0.3 ){
      h1_PtNewID_->Fill(Mu_ptcocktail->at(i),weight);
      h1_EtaNewID_->Fill(fabs(Mu_etaCocktail->at(i)),weight);
...
void ZprimeMuMuPat::MuonPassingTightID(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaCocktail->at(i)) < 2.4 &&
       ...
       Mu_numberOfMatchedStations->at(i) > 1 &&
       Mu_dPToverPTcocktail->at(i) < 0.3 ){
      h1_PtTightID_->Fill(Mu_ptcocktail->at(i),weight);
      h1_EtaTightID_->Fill(fabs(Mu_etaCocktail->at(i)),weight);
...
//                                                   -  
//----------------------------------------------------
bool ZprimeMuMuPat::RecoHLTMuonMatching(float RecoEta,float RecoPhi){
  int nbMatch = 0;
  ...
  if(event_runNo == runNumber && event_lumi == lumiNumber && event_evtNo == eventNumber)
    {
      output_txt << event_runNo
                 ...
                 << "        " << vtxMass << endl;
      for(unsigned i=0; i<Mu_nbMuon->size(); i++){
        //if( fabs(Mu_etaCocktail->at(i)) < EtaCut ){cout<<"[1] eta="<<Mu_etaCocktail->at(i)<<endl;}
	...
	if(Mu_numberOfValidMuonHits->at(i) > 0) {cout<<"[8] nbMuonHits="<<Mu_numberOfValidMuonHits->at(i)<<endl;}
        ...
        cout<<"[11] Charge="<<Mu_chargeCocktail->at(i)<<endl;
      }
