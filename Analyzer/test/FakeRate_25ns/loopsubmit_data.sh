...
while [ $n -lt ${nlines} ]; do
  ...
  if [ $4 = ${SCERN} ]; then
      ...
      cat submit_ZprimeMuMuAnalysis_FNAL.sh  | sed "s?path?$PATH?g"  | sed "s?lib?$LD_LIBRARY_PATH?g" | sed "s?which?data?g" | sed "s?site?$4?g" | sed "s?mc?$3?g" |sed "s?year?$2?g" | sed "s?jobdir?jobs/jobsZprimeMuMu_FR?g" | sed "s?histodir?histos/histosZprimeMuMu_FR?g" | sed "s?output?output_${samplename}?g" | sed "s?data_input.txt?data_input_${n}.txt?g" | sed "s?s.log?s_FR_${samplename}.log?g" > jobs/submit_ZprimeMuMuAnalysis_${samplename}.sh
      ...
      cat submit_ZprimeMuMuAnalysis_DESY.sh | sed "s?which?data?g" | sed "s?site?$4?g" | sed "s?mc?$3?g" |sed "s?year?$2?g" | sed "s?jobdir?jobs/jobsZprimeMuMu_FR?g" | sed "s?histodir?histos/histosZprimeMuMu_FR?g" | sed "s?output?output_${samplename}?g" | sed "s?data_input.txt?DataCards$2/data_input_${n}.txt?g" | sed "s?s.log?s_${samplename}.log?g" > jobs/submit_ZprimeMuMuAnalysis_${samplename}.sh
  elif  [ $4 = ${SBARI} ]; then
      ...
      cat submit_ZprimeMuMuAnalysis.sh | sed "s?which?data?g" | sed "s?mc?$3?g" |sed "s?year?$2?g" | sed "s?jobdir?jobs/jobsZprimeMuMu_FR?g" | sed "s?histodir?histos/histosZprimeMuMu_FR?g" | sed "s?output?output_${samplename}?g" | sed "s?data_input.txt?DataCards$2/data_input_${n}.txt?g" | sed "s?s.log?s_FR_${samplename}.log?g" > jobs/submit_ZprimeMuMuAnalysis_${samplename}.sh
