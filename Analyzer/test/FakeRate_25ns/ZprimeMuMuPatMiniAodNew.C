...
//                       25/5/2016                             = 
//==============================================================
#define ZprimeMuMuPatMiniAodNew_cxx
#include "ZprimeMuMuPatMiniAodNew.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include "TFile.h"
...
void ZprimeMuMuPatMiniAodNew::Loop()
{
time_t start,end;
double dif;
time (&start);
FILE * pFile;
   ...
   //TFile *output = new TFile("CMSSW803-Analyse_ZtoMuMu_13TeV_singleTop_miniaod.root","recreate");
   //TFile *output = new TFile("CMSSW803-Analyse_ZtoMuMu_13TeV_M120to200_miniaod2.root","recreate");
   TFile *output = new TFile("CMSSW803-TTTo2L2Nu_13TeV-powheg.root","recreate");
   //================================================================================== 
   //                                                                                 =
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	 ...
	 h1_PFMetCorr_->Fill(PFMet_pt_cor);
 ...
 //                                                   -  
 //----------------------------------------------------
 float ZprimeMuMuPatMiniAodNew::delR(float eta1,float phi1,float eta2,float phi2){
   float mpi=3.14;
 ...
 bool ZprimeMuMuPatMiniAodNew::GenRecoMatchMu(float RecoEta1,float RecoPhi1){
   int NbHEEPele = 0;
   unsigned iflag = -10;
 ...
 bool ZprimeMuMuPatMiniAodNew::SelectFirstMuon(float &pTmuon1,float &Enmuon1,float &Etamuon1,
 ...
 bool ZprimeMuMuPatMiniAodNew::SelectSecondMuon(int ChargeMu1,unsigned FlagMu1,float pTmuon1,float Etamuon1,float Phimuon1,
...
void ZprimeMuMuPatMiniAodNew::PlotRecoInfo(float CosmicMuonRejec, float vertexMassMu,float MassGenerated,
...
float ZprimeMuMuPatMiniAodNew::Mass(float Pt1,float Eta1,float Phi1,float En1,
...
void ZprimeMuMuPatMiniAodNew::PickThehighestMass(float &vtxHighestMass,float &vtxHighestChi2,int EvtNb)
{
  float Massinv  = -10.0;
  unsigned iflag = -10;
  vtxHighestMass = -10.0;
...
double ZprimeMuMuPatMiniAodNew::ThreeDangle(float pxMu1,float pyMu1,float pzMu1,float pMu1,
...
bool ZprimeMuMuPatMiniAodNew::SelectFirstGenMu(float &ETMu1,float &PhiSCMu1,
...
bool ZprimeMuMuPatMiniAodNew::SelectSecondGenMu(unsigned GenFlag1,float ETMu1,float &ETMu2,float &PhiSCMu2,
...
void ZprimeMuMuPatMiniAodNew::PlotGenInfo(float ZprimeGenMass,float EtaGenMu1,float EtaGenMu2,float PtGenMu1,
...
//                                                   -  
//----------------------------------------------------
void ZprimeMuMuPatMiniAodNew::MuonPassingID(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
...
void ZprimeMuMuPatMiniAodNew::PlotPterror(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAodNew::PlotNbTrackLayers(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAodNew::PlotNBValidPixelHits(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAodNew::PlotNbValidMuonHits(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAodNew::PlotNbMatchedStations(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAodNew::PlotTrackiso(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAodNew::PlotAbsDxy(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAodNew::PlotPtTuneP(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       Mu_absdxyTunePMuonBestTrack->at(i) < 0.2 &&
...
void ZprimeMuMuPatMiniAodNew::plotAllHighPtMuonsID(){
  MuonPassingID();
  PlotPterror();
  PlotNbTrackLayers();
...
void ZprimeMuMuPatMiniAodNew::MuonPassingNewID(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAodNew::MuonPassingTightID(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAodNew::CosThetaCollinSoper(float Et1,float Eta1,float Phi1,float En1,
...
void ZprimeMuMuPatMiniAodNew::PrintEventInformation(int runNumber, int lumiNumber, int eventNumber,
...
//                                                   -  
//----------------------------------------------------
bool ZprimeMuMuPatMiniAodNew::isPassHLT(){ 
  int nbMatch = 0;
...
bool ZprimeMuMuPatMiniAodNew::RecoHLTMuonMatching(float RecoEta,float RecoPhi){
  int nbMatch = 0;
...
bool ZprimeMuMuPatMiniAodNew::BTaggingDiscriminator(){
  int nbMatch = 0;
...
void ZprimeMuMuPatMiniAodNew::DrawBTaggingDiscriminator(){
  int nbMatch = 0;
  for(unsigned i=0; i<Nb_bDiscriminators->size(); i++){
    //printf ("[ %d ] flavor = %d disc_b = %f\n",i,jet_btag_flavor->at(i),jet_btag_pfCSVv2IVF_discriminator->at(i));
    //printf ("[ %d ] flavor = %d \n",i,jet_btag_flavor->at(i));
    h1_jetBTagAll_->Fill(jet_btag_pfCSVv2IVF_discriminator->at(i));
    if(jet_btag_flavor->at(i)==4) {h1_jetBTagC_->Fill(jet_btag_pfCSVv2IVF_discriminator->at(i));}
    else if(jet_btag_flavor->at(i)==5) {h1_jetBTagB_->Fill(jet_btag_pfCSVv2IVF_discriminator->at(i));}
    else {h1_jetBTagUDSG_->Fill(jet_btag_pfCSVv2IVF_discriminator->at(i));}
  }
}

/*
//===================== Methode to calculate the mass ========================
float ZprimeMuMuPatMiniAodNew::TransverseMass(float Pt1,float Eta1,float Phi1,float En1,
float Pt2,float Eta2,float Phi2,float En2){
float MuMuMass = 0.0;
TLorentzVector Mu1;
TLorentzVector Mu2;
...
*/
void ZprimeMuMuPatMiniAodNew::Boson(float Px1,float Py1,float Pz1,float En1,
