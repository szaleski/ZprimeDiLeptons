...
void ZprimeMuMu_FR::Loop()
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    h1_MET->Fill(PFMet_pt,weight);
    if (PFMet_sumEt>0.) h1_METSign->Fill(PFMet_pt/PFMet_sumEt,weight);
	...
	float dp=std::abs(PFMet_phi-Mu_phiTunePMuonBestTrack->at(i));
	if (dp>PI) dp-=float(2*PI);
	float mW_T=sqrt(2*Mu_ptTunePMuonBestTrack->at(i)*PFMet_pt*(1-cos(dp)));
	h1_mW_T->Fill(mW_T,weight);
	//if (mW_T>20 ) continue; // against W+jets
      }
...
float ZprimeMuMu_FR::Mass(float Pt1,float Eta1,float Phi1,float En1,
...
void ZprimeMuMu_FR::PrintEventInformation(int runNumber, int lumiNumber, int eventNumber,
  ...
  if(event_runNo == runNumber && event_lumi == lumiNumber && event_evtNo == eventNumber)
    {
      output_txt << event_runNo
		 ...
		 << "        " << vtxMass << endl;
      for(unsigned i=0; i<Mu_nbMuon->size(); i++){
	//if( fabs(Mu_etaCocktail->at(i)) < 2.5 ){cout<<"[1] eta="<<Mu_etaCocktail->at(i)<<endl;}
...
void ZprimeMuMu_FR::PickThehighestMass(float &vtxHighestMass,float &vtxHighestChi2,int EvtNb)
{
...
double ZprimeMuMu_FR::ThreeDangle(float pxMu1,float pyMu1,float pzMu1,float pMu1,
...
*/
bool ZprimeMuMu_FR::SelectFirstGenMu(float &ETMu1,float &PhiSCMu1,
...
bool ZprimeMuMu_FR::SelectSecondGenMu(unsigned GenFlag1,float ETMu1,float &ETMu2,float &PhiSCMu2,
...
//                                                   -  
//----------------------------------------------------
int ZprimeMuMu_FR::GenToRecoMatchMu(float pTgen, float etagen, float phigen){
...
bool ZprimeMuMu_FR::GenRecoMatchMu1(float RecoEta1,float RecoPhi1,
...
bool ZprimeMuMu_FR::GenRecoMatchMu2(unsigned GenFlag1,float RecoEta2,float RecoPhi2,
...
bool ZprimeMuMu_FR::SelectFirstMuon(float &pTmuon1,float &Enmuon1,float &Etamuon1,
...
bool ZprimeMuMu_FR::SelectSecondMuon(int ChargeMu1,unsigned FlagMu1,float pTmuon1,float &pTmuon2,float &Enmuon2,
...
bool ZprimeMuMu_FR::SelectSecondSSMuon(int ChargeMu1,unsigned FlagMu1,float pTmuon1,float &pTmuon2,float &Enmuon2,
				     float &Etamuon2,float &Phimuon2,int &ChargeMu2,float &pxmuon2,
				     float &pymuon2,float &pzmuon2,float &pmuon2,float &dxymuon2,
				     float &pTmuon2tuneP,float &pTmuonBestTrack2)
{
...
bool ZprimeMuMu_FR::RecoHLTMuonMatching(float RecoEta,float RecoPhi){
  int nbMatch = 0;
...
float ZprimeMuMu_FR::delR(float eta1,float phi1,float eta2,float phi2){
  float mpi=3.14;
